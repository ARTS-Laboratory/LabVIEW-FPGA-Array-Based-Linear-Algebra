...
#%% create the M matrix and save it to a file for the LabVIEW code

M,K = MK.make_M_K(beam_node_num = 5) # this gets a M and K matrix
...
#
#
#L_python_2 = np.copy(L_python)
#for i in range(L_python.shape[1]):
#    L_python_2[:,i] = L_python[:,i]/L_python[i,i]
#
#diff_2 = np.abs(L_python_2-L)
#
#def is_pos_def(x):
...
#is_pos_def(M)
#
#combined_MK = np.matmul(np.matmul(np.linalg.inv(L),K),np.linalg.inv(L.T))
#combined_MK_python = np.matmul(np.matmul(np.linalg.inv(L_python),K),np.linalg.inv(L_python.T))
#
#M_check = np.matmul(L,L.T)
#
