...
# index _ row _ column and is C[row*n+col] where row is multiplied by n
...
for i_rotation in range(max_rotation): # Jacobi rotation loop 
    ...
    for i_row in range(n-1):
        for j_col in range(n-1-i_row):
            ...
            if abs(C[index_i_ji1]) >= C_Max:
                C_Max = abs(C[index_i_ji1]) #i_row,j_col+i_row+1])
                k = i_row
                l = j_col+i_row+1
    index_k_k = k*n+k 
    index_l_l = l*n+l 
    C_Diff = C[index_l_l] - C[index_k_k] # difference of the orthogonal distance
    index_k_l = k*n+l
    if abs(C[index_k_l]) < abs(C_Diff)*1.0e-36:  # some sort of check
        t = C[index_k_l]/C_Diff
    else:   # calculate phi and t for the rotation
        phi = C_Diff/(2.0*C[index_k_l])
    ...
    temp = C[index_k_l]
    C[index_k_l] = 0.0
    C[index_k_k] = C[index_k_k] - t*temp
    C[index_l_l] = C[index_l_l] + t*temp
    # rotate around the high value point
    for i in range(k):      # Case of i < k
        index_i_k = i*n+k
        index_i_l = i*n+l
        temp = C[index_i_k]
        C[index_i_k] = temp - s*(C[index_i_l] + tau*temp)
        C[index_i_l] = C[index_i_l] + s*(temp - tau*C[index_i_l])
    for i in range(l-1-k):  # Case of k < i < l
        index_k_i1k = k*n+i+1+k
        index_i1k_l = (i+1+k)*n+l
        temp = C[index_k_i1k]
        C[index_k_i1k] = temp - s*(C[index_i1k_l] + tau*C[index_k_i1k])
        C[index_i1k_l] = C[index_i1k_l] + s*(temp - tau*C[index_i1k_l])
    for i in range(n-1-l):  # Case of i > l
        index_k_i1l = k*n+i+1+l
        index_l_i1l = n*l+i+1+l        
        temp = C[index_k_i1l]
        C[index_k_i1l] = temp - s*(C[index_l_i1l] + tau*temp)
        C[index_l_i1l] = C[index_l_i1l] + s*(temp - tau*C[index_l_i1l])
