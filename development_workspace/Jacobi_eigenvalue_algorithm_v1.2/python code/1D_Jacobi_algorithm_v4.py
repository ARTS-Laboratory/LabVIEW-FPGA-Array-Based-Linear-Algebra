...
#A_in_matrix = np.array([[1,2,0],[2,3,4],[0,4,5]],dtype=float)
A_in_matrix = np.array([[0.414,0.493,0.427],
                         [0.493,0.377,0.248],
                         [0.427,0.248,0.988]])

A_in_matrix = np.array([[0.8537870049477,0.8593040108681,0.2404720038176],
                         [0.8593040108681,0.8662329912186,0.2412869930267],
                         [0.2404720038176,0.2412869930267,0.06870730221272]])
...
scipy_eigvals = np.sort(scipy_eigvals)
...
number_of_rotations = 1  # Set limit on number of rotations, 5*(n**2) is a default number
...
for i_rotation in range(number_of_rotations): # Jacobi rotation loop 
    ...
    for i_row in range(n-1):
        for j_col in range(n-1-i_row):
            ...
            #print('A[index_i_ji1]) >= A_Max = '+str(A[index_i_ji1] >= A_Max))
    ...
    # print('A Diff = '+str(A_Diff)) 
    # print('A[index_k_l] = '+str(A[index_k_l]))    
    # print('index_k_l = '+str(index_k_l) ) 
    # print('abs(A[index_k_l]) < abs(A_Diff)*1.0e-36 = '+str(abs(A[index_k_l]) < abs(A_Diff)*1.0e-36)) 
    ...
    # print('A[index_k_k] - t*temp = '+str(A[index_k_k] - t*temp)) 
    # print('index_k_k = '+str(index_k_k)) 
    # print(np.reshape(A,(3,3)))  
    # break
    
    # print('A[index_k_k] = '+str(A[index_k_k]))
    # print('A[index_k_k] - t*temp = '+str(A[index_k_k] - t*temp))
    
    # print('A[index_k_k] = '+str(A[index_k_k]))
    # print('t*temp = '+str(t*temp))
    # print(np.reshape(A,(3,3))) 
    ...
    #sys.exit()
        # after kl is good, the after kk is bad. 
   ...
   # print(np.reshape(A,(3,3)))  
    ...
    # print('A after step 3.2')    
    # print(np.reshape(A,(3,3))) 
        
    for i in range(n-1-l):  # Case of i > l
        print('n-1-l = '+str(n-1-l))    
        print('k*n+i+1+l = '+str(k*n+i+1+l))
        index_k_i1l = k*n+i+1+l

        print('n*l+i+1+l  = '+str(n*l+i+1+l ))
        ...
        print('A[index_l_i1l]  = '+str(A[index_l_i1l]))
        print('s  = '+str(s))
        print('temp  = '+str(temp))
        print('tau  = '+str(tau))
        print('s*(temp - tau*A[index_l_i1l])  = '+str(s*(temp - tau*A[index_l_i1l])))
